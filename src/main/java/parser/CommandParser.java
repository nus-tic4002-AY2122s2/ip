...
public class CommandParser extends Parser {
    ...
    private Command parseCommand(String input) throws ErrorHandler {
        ...
        case DEADLINE:
            if (result.length < 2) {
                throw new ErrorHandler("In Parser, " + ErrorMessage.EMPTY_DEADLINE);
            }
        ...
        case EVENT:
            if (result.length < 2) {
                throw new ErrorHandler("In Parser, " + ErrorMessage.EMPTY_EVENT);
            }
        ...
        case TODO:
            if (result.length < 2) {
                throw new ErrorHandler("In Parser, " + ErrorMessage.EMPTY_TODO);
            }
            this.content = result[1].trim();
            return new TodoCommand(this.content, false);
        case DONE:
            if (result.length < 2) {
                throw new ErrorHandler("In Parser, " + ErrorMessage.EMPTY_TASK_NUMBER);
            }
            this.content = result[1].trim();
